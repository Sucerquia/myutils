...
while getopts 'o:p:n:rs:h' flag; do
    case "${flag}" in
      o) overstretching=${OPTARG} ;;
      p) pep=${OPTARG} ;;
      n) processors=${OPTARG} ;;
      r) restart='true' ;;
      s) size=${OPTARG} ;;
...
then
    ...
    myutils log2xyz $pep-stretched${nameiplusone}-a.log 2> /dev/null && \
        ...
        myutils sep_w_cons $pep-stretched${nameiplusone}-a-bck2.xyz \
    ...
    $retake && \
        ...
        myutils sep_w_cons $pep-stretched${nameiplusone}-bck1.xyz \
            $pep-stretched${nameiplusone} 0 1 0 $charge frozen_dofs.dat && \
        retake='false' && \
...
# ----- stretching starts -----------------------------------------------------
verbose "Stretching of $pep starts and will run until getting $overstretching
    ruptures"

while [[ $( cat frozen_dofs.dat | wc -l ) -lt $overstretching ]]
do
    # names by index
	namei=$(printf "%02d" $i)
	nameiplusone=$(printf "%02d" $(($i + 1)))
    ...
    then
        ...
        then
            myutils sep_w_cons \
                ...
                $index1 $index2 $size $charge frozen_dofs.dat \
        ...
        myutils sep_w_cons $pep-stretched00.pdb \
            $pep-stretched00 $index1 $index2 0 $charge frozen_dofs.dat || \

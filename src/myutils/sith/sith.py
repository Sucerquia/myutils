...
class Geometry:
    def __init__(self, xyz_file, force_file):
        ...
        indexes = np.loadtxt(force_file, usecols=1,
                             dtype=str)
        self.dim_indices = [eval(ric_indexes) for ric_indexes in indexes]
        self.dof = np.loadtxt(force_file, usecols=3)

        self.dims = np.zeros((4), dtype=int)
        for index in self.dim_indices:
            self.dims[0] += 1
        ...
        with open(force_file) as ff:
            header = ff.readline()
        self.scf_energy = float(header.replace('\n', '').split(' ')[-1])
    ...
    def _killDOFs(self, dofis):
        ...
        """
        self.dof = np.delete(self.dof, dofis)
        ...
        return self.dof, self.dims, self.internal_forces
...
class Sith:
    ...
    def _check_dofs(self):
        ...
        assert error.
        """
        referece = self.structures[0].dim_indices
        for defo in self.structures[1:]:
            to_compare = defo.dim_indices
    ...
    def simpson_integration(self):
        ...
        for angle in rics[self.dims[1]:]:
            for i in range(self.nstructures - 1):
        ...
        # be improved
        for i in range(1, self.nstructures):
...
# --------------------------- killer ------------------------------------------
    def killer(self, killAtoms=[], killDOFs=[], killElements=[]):
        ...
        self.dims_to_kill = [self.structures[0].dim_indices[i] for i in killDOFs]
        ...
        for atom in self.atoms_to_kill:
            self.dims_to_kill.extend(
                [dim for dim in self.structures[0].dim_indices if atom in dim])
        # remove repetitions
    ...
    def __killDOFs(self, dofs):
        rIndices = list()
        for dof in dofs:
            rIndices.extend([i for i in range(self.structures[0].dims[0])
                            if self.structures[0].dim_indices[i] == dof])
...
# ---------------------- remove deformations ----------------------------------
    def rem_first_last(self, rem_first_def=0, rem_last_def=0):
        ...
        last_index = self.nstructures - rem_last_def
        ...
        self.nstructures -= rem_first_def + rem_last_def

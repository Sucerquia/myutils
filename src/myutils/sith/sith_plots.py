...
def plot_averages_per_pos(ener_per_pos, mean_per_pos, stdr_per_pos,
    ...
    for i in [0, 1, 2]:
        ...
        factor = 0.92
        sp.ax[i].text(-0.5, ylim[-1]*factor + (1-factor) * ylim[0], f"Position {i+1}", fontsize=8, va='top',
    ...
    sp.spaces[0].set_axis(borders=[[0.105, 0.08], [0.99, 0.97]], spaces=(0.1, 0.07) , rows_cols=(3,1));
...
def plot_matrix(matrix, labels, n_per_ele, cbar_label):
    ...
    sp.ax[0].tick_params(top=True, bottom=False, labeltop=True, labelbottom=False)
    sp.ax[0].set_xticks(np.arange(matrix.shape[1]+1)-.5, minor=True)
    sp.ax[0].set_yticks(np.arange(matrix.shape[0]+1)-.5, minor=True)
    sp.ax[0].grid(which="minor", color=[0.4, 0.4, 0.4], linestyle='--', linewidth=0.3)
    ...
    for i in range(len(labels)):
        for j in range(len(labels)):
            color = "black"
            if matrix[j][i] > (np.nanmax(matrix.flatten()) + np.nanmin(matrix.flatten()))/2:
                color="white"
            sp.ax[0].text(i, j, int(n_per_ele.T[i][j]), va='center', ha='center', fontsize=5, color=color)
    ...
    sp.spaces[0].locate_ax([[left + 0.0, bottom+ (1-side)/2],
                            ...
                            ax=sp.ax[0])
    ## ColorBar
    sp.spaces[0].locate_ax([[0.76, bottom+ (1-side)/2], [0.82, 0.912]], ax=sp.ax[1])
...
class SithPlotter(PepSetter, SithAnalysis):
    ...
    def plot_energies_in_DOFs(self, steps: list = None, jump_stretching: int = 1,
        ...
        **kwargs:
        ...
        self.plot_sith(np.arange(1, dims[0] + 1), energies_per_DOF,
                       ...
                       show_amino_legends=True, jump_stretching=jump_stretching, **kwargs)
        ...
        self.plot_sith(np.arange(dims[1] + dims[2] + 1, dims[0] + 1),
                       ...
                       step=steps[3], sp=sp, jump_stretching=jump_stretching, **kwargs)
    ...
    def plot_sith(self, dofs: Union[list, tuple, np.ndarray] = None,
        ...
        if cbar:
            ...
            cbar.set_label(label="Stretched",
                           fontsize=mpl.rcParams['font.size'] * sp.ax_pref['labels_scale'],
                           rotation=90)
        ...
        if show_amino_legends:
            ax.legend(handles=list(colors.values()), loc='upper right', fontsize=8)
    ...
    def _dof_classificator(self):
        ...
        for i in range(len(dofs_indexes)):
            for j in atoms_per_aminoacids.keys():
                if np.isin(dofs_indexes[i], atoms_per_aminoacids[j] + [0]).all():
    ...
    def plot_error(self,
        ...
        sp.plot_data(distances, e[0], ax=0, data_label='SITH', pstyle='o', fillstyle='none', ms=5)

...
then
  ...
  awk 'BEGIN{print "time"}{print $1}' tmp_analyzer_potential.dat > \
    ...
    awk 'BEGIN{print "e_potential"}{print $2}' tmp_analyzer_potential.dat > \
      tmp_remove.dat && \
    [ "$( wc -l < tmp_remove.dat )" -gt 2 ] || fail "Extracting energy data"
...
then
  ...
  sed "s/@/#/g" tmp_energies.xvg > "analysis_protein_energies-$name.dat" && \
    ...
    [ "$( wc -l < tmp_analyzer_pep_potential.dat )" -gt 2 ] || fail "Summing
      potential energies"
...
then
  ...
  grep -v "\#" "analysis_distance-$name.dat" > tmp_analyzer_distance.dat &&
    awk 'BEGIN{print "time"}{print $1}' tmp_analyzer_distance.dat > \
      tmp_analyzer_time.dat && \
  [ "$( wc -l < tmp_analyzer_time.dat )" -gt 2 ] || fail "Extracting time"
  awk 'BEGIN{print "distance"}{print $2}' tmp_analyzer_distance.dat > \
      tmp_remove.dat && \
    [ "$(wc -l < tmp_analyzer_distance.dat)" -gt 2 ] || fail "Extracting
      distance"
...
then
  ...
  do
    echo " merge $field"
    paste "analysis_merged_table-$name.dat" "tmp_analyzer_$field.dat" | \
      column -t > tmp_analyzer_remove.dat
...
then
  echo -e "1\n" | $gmx trjconv -s "$name.tpr" -f "$name.trr" -dump 500 -o \
    "analysis_lastconfig-$name.pdb" -pbc mol > "$output" 2>&1 || \
    fail "extracting last configuration"
...
if [ ! $keep ]
then
  count=$( find . -name "\#*" 2>/dev/null | wc -l )
  if [ "$count" -ne 0 ];
  then

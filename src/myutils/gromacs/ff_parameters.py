...
def create_bondpars_file(outfile, bond_idxs, bond_eqs, bond_ks):
    ...
    bond_indxs: list
        set of bonds defined as [atom1, atom2].
    bond_eqs: list
        set of equilibrium values of the bonds.
    bond_ks: list
        set of harmonic constants.
...
def grappa_bond_pars(pdb):
    """
    Extracts the bond parameters predicted by grappa.

    Parameters
    ==========
    pdb: str
        Molecule for which you want to predict the parameters.

    Return
    ======
    (tuple) bond idices, equilibrium distances, harmonic constants.
    """
...
def amber_bond_pars(pdb):
    """
    Extracts the bond parameters predicted by amber.
